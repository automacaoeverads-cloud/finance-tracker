  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-0.5">Gerencie quem pode ser vinculado a um gasto</p>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100"
        ...
        <form onSubmit={handleAdd} className="flex gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-blue-300 bg-slate-50/60 text-slate-700 placeholder:text-slate-400"
            required
          />
          <button
            type="submit"
            disabled={adding || !newName.trim()}
            className="px-6 py-3 rounded-xl bg-blue-500 text-white text-sm font-semibold hover:bg-blue-600 transition-colors disabled:opacity-70 flex items-center gap-2 shadow-sm"
          >
            {adding ? (
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100"
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-slate-700">Pessoas Cadastradas</h3>
        ...
        {loading ? (
          ...
          <ul className="space-y-2">
            {people.map(person => (
              <li
                key={person.id}
                className="flex items-center justify-between px-4 py-3 rounded-xl border border-slate-100 hover:bg-blue-50/30 transition-colors group"
              >
